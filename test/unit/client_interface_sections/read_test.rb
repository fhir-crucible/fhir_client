...
class ClientInterfaceReadTest < Test::Unit::TestCase
  ...
  def test_read
    ...
    temp.use_stu3
    temp.default_json
    reply = temp.read(FHIR::STU3::Patient,'foo')
  ...
  def test_vread
    ...
    temp.use_stu3
    temp.default_json
    reply = temp.vread(FHIR::STU3::Patient,'foo', 2)
    assert reply.resource.is_a?(FHIR::STU3::Patient)
    assert reply.resource_class == FHIR::STU3::Patient
    assert reply.id == 'foo'
    check_header_keys reply
  ...
  def test_conditional_read_since
    ...
    temp.use_stu3
    temp.default_json
    reply = temp.conditional_read_since(FHIR::STU3::Patient,'foo', 'Wed, 21 Oct 2015 07:28:00 GMT')
    assert reply.resource.is_a?(FHIR::STU3::Patient)
    assert reply.resource_class == FHIR::STU3::Patient
    assert reply.id == 'foo'
    check_header_keys reply
  ...
  def test_conditional_read_version
    ...
    temp.use_stu3
    temp.default_json
    reply = temp.conditional_read_version(FHIR::STU3::Patient,'foo','ABC')
    assert reply.resource.is_a?(FHIR::STU3::Patient)
    assert reply.resource_class == FHIR::STU3::Patient
    assert reply.id == 'foo'
    check_header_keys reply
  ...
  def test_raw_read
    ...
    temp.use_stu3
    temp.default_json
    options = {resource: FHIR::STU3::Patient, id: 'foo'}
    reply = temp.raw_read(options)
    returned_resource = temp.parse_reply(FHIR::STU3::Patient, FHIR::Formats::ResourceFormat::RESOURCE_JSON, reply)
    assert returned_resource.is_a?(FHIR::STU3::Patient)
